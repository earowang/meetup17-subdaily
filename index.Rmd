...
```{r flinders-2016-plot, echo = FALSE, fig.height = 11}
```
...
.right-column[
### Lagged scatterplot on a "daily" calendar
```r
...
# tsibble
## .blue[`%>%`]
## Australian English for time series tibble
...
* `tbl_forcast`: tsibble for forecast
* `expand`: make implict missing cases to explict
* `slide`: rolling window calculation

### Graphical methods for tsibble in `sugrrants`
### Forecast methods for tsibble in `forecast`, `hts` and `fastter`
]
